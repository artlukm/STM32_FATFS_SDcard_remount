...
)
{
	...
	do {
		res = xchg_spi(0xFF);			// ошибка после извлечения
	} while ((res & 0x80) && --n);
...
)
{
	...
	if (count == 1) {	/* Single sector read */
		if ((send_cmd(CMD17, sector) == 0)	/* READ_SINGLE_BLOCK */	// Start sector number 0. count = 1
	...
	return count ? RES_ERROR : RES_OK;	/* Return result */	// count = 1
}
